...
public class MainShowGYR extends Activity implements SensorEventListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_gyr);
        ...
        mService.setXYMultipleSeriesDataset("gyr","gyr","gyr");
        mService.setXYMultipleSeriesRenderer(0, 100, 0, 10, "gyr", "time", "m/s^2",
    ...
    private double gyr = 0;
    @Override
    public void onSensorChanged(SensorEvent event) {
        final float[] values = event.values;
        gyr = Math.sqrt(values[0]*values[0]+values[1]*values[1]+values[2]*values[2]);
        //int t = 0;
        // handler = new Handler() {
        //定时更新图表
        // public void handleMessage(Message msg) {
        if (delay == 50){
            mService.updateChart(t, gyr, gyr, gyr);
